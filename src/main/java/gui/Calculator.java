...
public class Calculator extends VBox implements EventHandler<ActionEvent> {
    ...
    public void handle(ActionEvent event) {
        ...
        if (value.matches("[0-9]")) {
            // Es un número, concaténalo al número actual
            if (operator.isEmpty()) {
                number1 += value;
            } else {
                number2 += value;
            ...
            // Es un operador, guárdalo
            if (number1.isEmpty()) {
                // No hay primer número, no se puede establecer un operador
                return;
            }
            operator = value;
        } else if ("=".equals(value)) {
            // Es el botón de igual, realiza la operación
                ...
                switch (operator) {
                    ...
                    case "/":
                        if (num2 != 0) {
                            ...
                            this.displayText.setText("Error: División por cero");
                            return;
                        }
                        break;
                }
